  ...
  PRIVATE SECTION.
    ...
    METHODS get_requests
      IMPORTING
        !it_part          TYPE ztimem_part_t
      RETURNING
        VALUE(rt_request) TYPE ztimem_request_info_t .
    METHODS get_stats
      IMPORTING
        !it_part        TYPE ztimem_part_t
      RETURNING
        VALUE(rs_stats) TYPE ztimem_stats .
...
CLASS ZCL_TIMEM_PARTS IMPLEMENTATION.
  ...
  METHOD get_authors.
    ...
    DATA(t_line) = VALUE ztimem_line_t( FOR s_part IN it_part
  ...
  METHOD get_data.
    DATA(t_part) =
      VALUE ztimem_part_t( FOR o_part IN gt_part
                           ( name = o_part->name
  ...
  METHOD get_stats.
    DATA(t_line) = VALUE ztimem_line_t( FOR s_part IN it_part
                                 FOR s_line IN s_part-t_line
                                 ( s_line ) ).
    rs_stats = NEW zcl_timem_stats( t_line )->stats.
  ENDMETHOD.
  ...
  METHOD load.
    DATA(o_object) = NEW zcl_timem_object_factory( )->get_instance( i_object_type = me->g_type
